...
2. Read **content.json** — this is the **content ground truth**. It mirrors the section structure: `{ header, main: [section, …], footer }`. Each main entry has `id`/`role`/`tag`/`className` plus its own headings, paragraphs, lists, links, images, forms, buttons, tables, definitionLists, and labelValuePairs. The `id` field matches the screenshot filename in the section dirs (e.g. `id: "hero"` ↔ `NN-hero.png`). All text, links, image src/alt, headings, and button labels MUST come from here, verbatim. Do **not** invent copy.
   ...
   - **`videos`** — one entry per rendered `<video>` or player `<iframe>`: `{ kind, src, poster, title, width, height, autoplay }` (`kind` is `video`, `youtube`, `vimeo`, `wistia`, `loom`, or `iframe`). Render `kind: "video"` as `<video controls src="…" poster="…">` and every other kind as `<iframe src="…" title="…" class="aspect-video w-full" loading="lazy" allowfullscreen></iframe>`. Never drop one and never substitute a placeholder image — a missing video fails `verify-structure`.
...
- **Inline `<header>` / `<footer>` from `content.json` (no Twig).** Page-mode drafts must open under `file://` for local verify. Write a real `<header>…</header>` and `<footer>…</footer>` from the captured chrome content. Do **not** emit Twig `wpcanai_template` includes — handoff will swap them later. *If the prompt's This page block says chrome is skipped, the `## Chrome: skipped` block at the end overrides the inline-chrome rule.*
