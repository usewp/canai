...
Three helper families are available: **CanAI native i18n** (`t()`, `tmedia()`, `current_lang()`, `languages()`, `lang_url()`, plugin 1.22+ — active for the single-post translation model), **Polylang-only** (`current_language()`, `language_switcher()`), and the **WordPress gettext** family (`__()`, `_x()`, `_n()`).
...
**Native i18n — global strings & automatic SEO (plugin 1.30+):** beyond `t()` string translations and per-post/term content overrides, a per-language **global-string** store (option `wpcanai_i18n_global_strings`, service `WPCanAI\I18n\GlobalStrings`) holds site name, tagline, and archive/search/404 SEO title+description (archive contexts `search`, `404`, `post_type:<slug>`) — these feed `option_blogname` / `option_blogdescription` + `og:site_name` and the archive `<title>` / meta description. On non-default pages the SEO bridge also auto-derives missing `seo_title`/`seo_description` from translated content (description ≤160 chars on a word boundary), `/<lang>/`-prefixes canonical + `og:url` (suppressing CanAI's own canonical when Slim SEO / RankMath is active), preserves the `/<lang>/` prefix on redirects except WordPress system paths, and translates `Article.headline` in JSON-LD. See SKILL.md → native translation. Term archives (category / tag) use per-term overrides, not global strings.
...
Loop/archive products expose `product.wc` when enriched via `wpcanai_get_posts_enriched` with `wpcanai_include=featured_image`. On a **single-product** page taken over by CanAI, the queried `post` also gets this same surface with no include needed — see **Single-product `post.wc`** below.
