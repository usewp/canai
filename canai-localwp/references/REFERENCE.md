...
**Template variables:** If you mark `products` or `posts` as *required* in template settings, an **empty list** is still valid (for example a valid page with no rows). Required means the variable is **present**, not that the array is non-empty.
...
Three helper families are available: **WPCanAI native i18n** (`t()`, `tmedia()`, `current_lang()`, `languages()`, `lang_url()`, plugin 1.22+ — active for the single-post translation model), **Polylang-only** (`current_language()`, `language_switcher()`), and the **WordPress gettext** family (`__()`, `_x()`, `_n()`).

```twig
{{ t('Shop now') }}                     {# translated via the site-wide string table #}
<img src="{{ tmedia(123) }}">           {# per-language attachment for id 123 #}
{% for l in languages() %}
  <a href="{{ lang_url(l.slug) }}" {{ l.slug == current_lang() ? 'aria-current="true"' }}>{{ l.native_name }}</a>
{% endfor %}
```

Polylang-only helpers and the WordPress gettext family are active whether or not Polylang is installed — `current_language()` falls back to `get_locale()`, and `language_switcher()` returns `[]`. Strings without an explicit text domain default to `wpcanai`.
...
**Native i18n — global strings & automatic SEO (plugin 1.30+):** beyond `t()` string translations and per-post/term content overrides, a per-language **global-string** store (option `wpcanai_i18n_global_strings`, service `WPCanAI\I18n\GlobalStrings`) holds site name, tagline, and archive/search/404 SEO title+description (archive contexts `search`, `404`, `post_type:<slug>`) — these feed `option_blogname` / `option_blogdescription` + `og:site_name` and the archive `<title>` / meta description. On non-default pages the SEO bridge also auto-derives missing `seo_title`/`seo_description` from translated content (description ≤160 chars on a word boundary), `/<lang>/`-prefixes canonical + `og:url` (suppressing WPCanAI's own canonical when Slim SEO / RankMath is active), preserves the `/<lang>/` prefix on redirects except WordPress system paths, and translates `Article.headline` in JSON-LD. See SKILL.md → native translation. Term archives (category / tag) use per-term overrides, not global strings.

**Polylang-model storage rules** (see SKILL.md → Multi-language — Polylang model): `wpcanai_template` posts are translatable; `_canai_html/css/js/context/context_mode` are **copied** on translation; `_canai_layout` and `_canai_delegate_page_id` are **translated** (per-language IDs); the `template_type` taxonomy is **not** translatable.
...
<!-- parity: keep in sync with canai-mcp REFERENCE §WooCommerce Functions -->
...
### Product Enrichment (loop context)

Loop/archive products expose `product.wc` when enriched via `wpcanai_get_posts_enriched` with `wpcanai_include=featured_image`. On a **single-product** page taken over by WPCanAI, the queried `post` also gets this same surface with no include needed — see **Single-product `post.wc`** below.
